...
- **Bulk export (`POST /v3/export` / `POST /v3/export/presigned_url`)**. This endpoint isn't documented anywhere on shopmonkey.dev (we couldn't find it as of 2026-04). We've used it successfully for full-shop reconciliation. We'd much rather it be a first-class, documented endpoint with parity to the REST schema. Today there are minor field divergences between export and REST (e.g., `fee.percentValue` vs `fee.percent`, `subcontract.wholesaleCostCents` vs `subcontract.costCents` — though see test 11 for the latter). Calling an undocumented endpoint from a public test harness felt presumptuous, so this is documented but not exercised.
