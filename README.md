...
| 03 — `limit` silently clamped | `?limit=500` returns exactly 100 rows. No `4xx`, no warning header. The 100-row cap is not documented anywhere. |
...
4. **Behavior when `limit > 100`.** Either return more rows or return a `400`. Silent clamping (test 03) is the surprising case.
5. **EDS (Enterprise Data Streaming) availability outside of HQ.** [`shopmonkeyus/eds`](https://github.com/shopmonkeyus/eds) would solve essentially every issue in this report — it's CDC, no pagination, no filtering. Today it's gated behind enterprise tier. Even read-only access at lower tiers would be transformative.
6. **Bulk export endpoint docs.** [`shopmonkey.dev/resources/export`](https://shopmonkey.dev/resources/export) is marked WIP with no prose summary, no list of available `tables`, no parity statement against the REST schema, and no mention of freshness lag or that soft-deleted rows are included. We've integrated against it successfully but the doc is thin.
